...
class Leaderboard:
    ...
    def eval_match(self, match: MatchResultDouble):
        ...
        a_elo = (self.player_points[match.a_goalie]['elo'] + self.player_points[match.a_striker]['elo']) / 2
        b_elo = (self.player_points[match.b_goalie]['elo'] + self.player_points[match.b_striker]['elo']) / 2

        a_delta, b_delta = self.elo_system.delta_score(a_elo, match.a_score, b_elo, match.b_score)
